...
6. **Source-attributed.** Every price-carrying message carries a `u16` source ID. With a single publisher this is redundant; with many it is essential. `Source ID` distinguishes *sources*, not publisher *hosts*; redundant hosts of one channel share it and are told apart by other means ([Redundant Publisher Hosts](#redundant-publisher-hosts)).
...
| 20 | Snapshot ID | `u32` | Monotonically increasing per `(channel_id, instrument_id)` within the current `Reset Count` era, **and per publisher host** ([Redundant Publisher Hosts](#redundant-publisher-hosts)). Identifies this snapshot instance, so that subscribers can associate `SnapshotOrder` messages with the correct `SnapshotBegin` and detect stale or out-of-order snapshot fragments. |
...
Existing field layouts and semantics will not change within the `3.x` line. The following are known candidates that would be **breaking**, and each requires a MAJOR release and a Schema Version bump:
