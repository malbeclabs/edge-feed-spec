...
6. **Source-attributed.** Every price-carrying message on the market data path carries a `u16` Source ID. With a single publisher this is redundant; with many it is essential. Snapshot-path messages omit it: a snapshot group is scoped to one `(channel_id, instrument_id)`, which identifies one book from one Source ID.
...
**3.0.1** — editorial. Removed the *Relationship to Sibling Feeds* enumeration, qualified the bare uses of "source", and adopted the glossary's "published set". No wire change.
