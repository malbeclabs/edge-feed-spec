...
- **Reduce** the depth constant *N* (requires a Schema Version bump, since it changes `BookDepth`'s length). *N* cannot be **raised** under the current framing at all: `BookDepth` is `24 + 40N` bytes and the `u8` `Message Length` cap of 255 puts the ceiling at `N = 5`, where this version already sits. A deeper book requires the framing accommodation reserved at type ID `0x41` — a new message type, not a longer `0x40` — because a length that does not fit `u8` cannot be expressed in the shared application message header at all. See [Depth Parameter](#depth-parameter).
