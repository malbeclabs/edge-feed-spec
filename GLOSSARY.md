...
**Do not resolve that by adopting a venue's vocabulary, because the venues do not share one.** Kalshi groups event contracts under a series and calls the whole product event contracts. Polymarket, checked against its live Gamma API, has no single-valued equivalent at all: an `event` groups one or more `markets`, each a Yes/No question, and classification is a many-to-many `tags` array of `{id, label, slug}` — an open-ended set running to thousands of entries, from `Crypto` down to one athlete's name. A field that must hold one value per feed row cannot follow a venue whose own scheme holds several per event and admits new ones continuously. `category` is therefore ours, and the case for keeping it ours is that it has a job neither venue's taxonomy is doing: deciding which rows arbitrate against each other.
