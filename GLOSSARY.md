...
| **Category** | The partition of a venue's instruments that a feed row carries, named by a string in the feed registry (`perps`, `sports`, `elections`). Rows sharing a `(venue, category)` are mirrors of one another and arbitrate as one; rows differing in it carry disjoint instrument sets and never contest each other's tape or book. | A matching engine, a channel, a product line, a venue's own naming for its markets |
...
**`Category` is an arbitration boundary, not a taxonomy.** It exists because one `Source ID` — and therefore one venue name — can span instrument sets that share nothing, and the venue-wide tape gate elects a single publisher across a venue: without the partition it elects one across both sets and drops the other's trades entirely, which reaches a subscriber as empty candles indistinguishable from a market that did not trade. What follows from that is the rule for choosing a value: two rows need different categories exactly when they must not arbitrate against each other, and the same one exactly when they are mirrors. It is not a place to describe what a market is about, and a value that reads like a description invites exactly that use.

The current values do read that way, and mix two axes: `perps` names an instrument type while `sports` and `elections` name event themes, so a venue's perpetuals and its event contracts are separated by the same field that separates two kinds of event contract from each other. Kalshi's own term for the second group is **event contracts**; `sports` and `elections` are themes within it.

**Do not resolve that by adopting a venue's vocabulary, because the venues do not share one.** Checked against their live interfaces, prediction venues fall into two camps and neither matches the shape of this field. Polymarket and Manifold tag many-to-many from an open set: a Polymarket `event` groups `markets` and carries a `tags` array of `{id, label, slug}` running to thousands of entries, from `Crypto` down to one athlete's name; a Manifold market carries `groupSlugs`, user-created and equally open. Betfair, the oldest of them, does the opposite — a closed operator-assigned hierarchy, `EventType` → `Competition` → `Event` → `Market`, single-valued at every level. Kalshi groups event contracts under a series and calls the product event contracts.

A field holding one value per feed row cannot follow the first camp, which holds several per event and admits new ones continuously. It *could* follow the second, and that is the one worth learning from — but Betfair's single-valued top level is a **domain** (`Soccer`, `Politics`), and `perps` is not a domain. Adopting a domain axis consistently would mean this field stops describing perpetuals at all, which is a larger change than a rename.

`category` is therefore ours. The case for keeping it ours is that it has a job none of these taxonomies performs: deciding which rows arbitrate against each other. The case for reviewing the values is that they currently imply a domain axis they do not implement.

What is worth settling is narrower: whether the values should be drawn from a fixed set stated here, so that a new registry row cannot invent a third axis, and whether the existing three should be reworded to stop reading as descriptions. That is a change to an interface — `category` reaches consumers through `/v1/status` channel rows, shows up in their filters, and travels in the URLs they share — so it is sequenced rather than renamed.
...
**1.4.0** — defined **Category**, the feed-registry field partitioning a venue's instruments, and stated what it is for: it is an arbitration boundary rather than a taxonomy, and the rule for choosing a value follows from that. Classified `MINOR` because it names a term already in use across the registry, `/v1/status` and consumer filters without redefining anything; the open question it records — whether the values should be drawn from a fixed set — is raised rather than settled, since changing one is an interface change and not a rename. Records that following a venue's own vocabulary is not the answer: Kalshi and Polymarket do not share a scheme, and Polymarket's has no single-valued equivalent to follow.
