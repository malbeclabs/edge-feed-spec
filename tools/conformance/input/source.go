...
// it never wrote — and a legacy pcap has no field for it. A packet that arrived
// with fewer bytes than its own UDP header declares is capture-owned loss in
// either format, and both report it here.
type CaptureLossReporter interface {
	...
	PendingDrops() uint64
	// SnaplenTruncated returns how many of the drops are packets that arrived
	// short of their own declared length — a snap length shorter than the feed's
	// datagrams — rather than ones the capture admits it never wrote. It changes
