...
// is the reason to replay the archive itself — but a cut leaves the same mark in
// either format: a UDP header declaring more payload than the record holds.
// Handing those bytes to the decoder charges the recorder's own snap length to
// the publisher, so the check lives in the reader both formats go through rather
// than in the pcapng one.
