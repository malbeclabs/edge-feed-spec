...
// wire than it holds. Both lengths are the format's, and only one of them is the
// number of bytes the block actually carries: a snap length shorter than the
// feed's datagrams produces this, and so does a writer counting the FCS.
...
// not the publisher's.
//
// The surviving bytes used to go to the decoder, where a datagram declaring a
// length past the bytes it holds reads as publisher truncation and a payload
// hashed short of its twin reads as a divergent duplicate. That is the
// recorder-artifact-charged-to-the-feed error epb_dropcount was threaded in to
// prevent, from a shortfall the reader could already see.
...
// A writer that reports a wire length above the bytes it handed over has
// truncated nothing, and discarding its packets discards the whole capture.
//
// The Ethernet FCS is the standard case: four bytes counted on the wire and
// stripped before userspace, which pcapng acknowledges with if_fcslen. A guard
// reading the record's captured length against its original length cannot tell
// that from a snap-length cut — it loses every packet, and the run exits 0 over
// an empty report while the warning blames a snap length that was fine. The UDP
// header's own length names the bytes the decoder needs, and every one of them
// is in the block.
func TestPcapngWireLengthAboveTheCapturedBytesIsNotTruncation(t *testing.T) {
	b := newNgBuilder(binary.LittleEndian)
	b.sectionHeader()
	b.iface(layers.LinkTypeEthernet, 65535)
	payloads := []string{"first whole", "second whole", "third whole"}
	for i, pl := range payloads {
		whole := udpEthPacket(t, i, 7001, []byte(pl))
		// original length counts four bytes of FCS the block does not carry.
		b.enhancedShort(0, uint64(i), whole, len(whole)+4)
	}
	...
	if len(got) != len(payloads) {
		t.Fatalf("got %d datagram(s), want %d: a wire length over the captured bytes is not "+
			"missing bytes, and discarding on it reports a clean run over nothing checked",
			len(got), len(payloads))
	}
	for i, pl := range payloads {
		if string(got[i].Raw) != pl {
			t.Errorf("datagram %d is %q, want %q", i, got[i].Raw, pl)
		}
	}
	if n, tr := src.CaptureDrops(), src.SnaplenTruncated(); n != 0 || tr != 0 {
		t.Errorf("CaptureDrops()=%d SnaplenTruncated()=%d, want 0 and 0: nothing was lost", n, tr)
	}
}

// A snap length short enough to cut the headers themselves is still the
// capture's loss, and must not read as a capture of nothing.
//
// Such a packet names no port, so the port map cannot be consulted for it and
// nothing separates it from traffic that was never UDP — except the decoder,
// which reports walking off the end of a header. Without that the loudest
// misconfiguration there is produces the quietest result there is: every packet
// skipped, zero findings, exit 0.
func TestPcapngPacketCutInsideItsHeadersIsCaptureOwned(t *testing.T) {
	whole := udpEthPacket(t, 0, 7001, []byte("recorded whole"))

	b := newNgBuilder(binary.LittleEndian)
	b.sectionHeader()
	b.iface(layers.LinkTypeEthernet, 20)
	b.enhancedShort(0, 0, whole[:20], len(whole)) // cut inside the IPv4 header
	b.enhanced(0, 1, whole, nil)
	...
	if got[0].CaptureDrops != 1 || src.SnaplenTruncated() != 1 {
		t.Errorf("drops=%d truncated=%d, want 1 and 1: a packet cut before its port is "+
			"legible is the capture's loss, not a packet that was never UDP",
