...
The tool is correct (no false `Violation`s) and complete for its primary use case: validating a feed captured from the publisher's **session start**. One boundary is deliberately conservative — it fails toward `Unverifiable`, never toward a false violation:
