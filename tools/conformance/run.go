...
// archive directly. A packet that arrived with fewer bytes than its own UDP
// header declares is counted in either format, and named separately here: an
// operator fixes it by re-recording with a snap length that holds the feed,
// which is not what they would do about a dropped datagram.
	...
	if t := cl.SnaplenTruncated(); t > 0 {
		fmt.Fprintf(os.Stderr,
			"dz-conformance: WARNING %d of those are datagrams the capture's snap length cut "+
				...
				"that holds the feed's datagrams.\n", t)
	}
	return n
}
...
func Run(opts RunOpts) int {
	...
	for {
		...
		// Before the datagram, not after: the drops precede it, so the windows
		// they taint are the ones it is about to be judged against.
