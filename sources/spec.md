...
This document specifies version **1.1.0**: the reserved ranges, the current assignment set, and the process for requesting a new ID.
...
| ID | Name | Code | Operator | Kind | Notes |
|----|------|------|----------|------|-------|
...
`Operator` is the venue running the engine, in the [glossary](../GLOSSARY.md)'s sense. It is recorded separately from `Name` so that several engines at one venue are legible as such.

`Name` on IDs `1` through `3` predates the rule that an ID names an engine, and each is currently the venue's name because each venue published one engine when its ID was assigned. Those names stand, because assigned rows are stable. A new row names the **engine**, not the operator, and where a venue runs more than one engine each takes its own row and its own uppercase `Code`.
...
2. Names the matching engine in `Name`, gives its uppercase `Code`, names the venue running it in `Operator`, and fills in `Kind` and (optionally) `Notes`.
...
Assigning a new Source ID is an additive change and is therefore a **MINOR** release, as is adding a column to the assignment table. Editorial changes to the `Name`, `Code`, `Operator`, `Kind`, or `Notes` of an existing row, and changes to this document's prose, are **PATCH** releases.
